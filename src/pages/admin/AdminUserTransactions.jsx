...
const isImageOrderField = (field = {}) => {
  ...
  return type === 'image' || UPLOAD_IMAGE_PATH_REGEX.test(value) || IMAGE_PATH_REGEX.test(value);
};

const getOrderFieldRows = (order = {}) => {
  const values = (
  ...
  const snapshots = Array.isArray(order?.customerInput?.fieldsSnapshot)
    ? order.customerInput.fieldsSnapshot
    : [];
  const rows = [];
  const seen = new Set();

  snapshots.forEach((field, index) => {
    const key = String(field?.key || field?.name || field?.id || `field_${index}`).trim();
    if (!key || !Object.prototype.hasOwnProperty.call(values, key)) return;
    rows.push({
      key,
      label: String(field?.label || field?.labelAr || key).trim(),
  ...
  Object.entries(values).forEach(([key, value]) => {
    if (seen.has(key) || value === undefined || value === null || String(value).trim() === '') return;
    rows.push({
      key,
      label: key,
...
const OrderFieldValue = ({ field, isArabic }) => {
  if (!isImageOrderField(field)) {
    return <span className="break-all font-semibold text-[var(--color-text)]">{field.value || '-'}</span>;
  }

  const href = resolveImageUrl(field.value);
  return (
    <a
      href={href}
      target="_blank"
              ...
              {detailsItem.source === 'wallet' ? (
                ...
                <div className="space-y-2 text-sm text-[var(--color-text)]">
                  ...
                  <div className="space-y-1.5">
                    <span className="text-[var(--color-text-secondary)]">{isArabic ? 'الحقول:' : 'Fields:'}</span>
                    {getOrderFieldRows(detailsItem.raw).length ? (
                      <div className="mt-1 grid gap-2 sm:grid-cols-2">
                        {getOrderFieldRows(detailsItem.raw).map((field) => (
                          <div key={field.key} className="rounded-lg border border-[color:rgb(var(--color-border-rgb)/0.75)] bg-[color:rgb(var(--color-surface-rgb)/0.45)] p-2">
                            <p className="mb-1 text-[11px] text-[var(--color-text-secondary)]">{field.label}</p>
                            <OrderFieldValue field={field} isArabic={isArabic} />
                          </div>
                        ))}
                      </div>
                    ) : (
